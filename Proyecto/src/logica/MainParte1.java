...
public class MainParte1 {
    public static void main(String[] args) {
        ...
        System.out.println("\n· Categoría automática (debe ser Documento (auto)): " + a1.getCategoria());

        // 2) Pruebas de esOculto() y esValido()
        Archivo oculto = new Archivo(".oculto", "C:/ruta/oculto.txt", "txt", 100, LocalDateTime.now(),
                LocalDateTime.now());
        System.out.println("\n· esOculto() de '.oculto': " + oculto.esOculto());
        System.out.println("· esValido() de '.oculto': " + oculto.esValido());
        Archivo invalido = new Archivo("   ", "C:/ruta/blank.txt", "txt", 100, LocalDateTime.now(),
                LocalDateTime.now());
        System.out.println("· esValido() de nombre en blanco: " + invalido.esValido());
        ...
        try {
            ...
            System.out.println("Tras agregar t1 y t2 (y t1 duplicada): " + a1.getEtiquetas());

            // Renombrado válido
            t1.setNombre("informe 2025");
            System.out.println("Tras renombrar t1 a 'informe 2025': " + a1.getEtiquetas());

            // Longitud >50 -> excepción
            String largo = "x".repeat(51);
            try {
                Etiqueta.crear(largo);
                System.out.println("ERROR: etiqueta de longitud >50 no lanzó excepción");
            } catch (IllegalArgumentException ex) {
                System.out.println("  [OK] Detectada etiqueta demasiado larga: " + ex.getMessage());
            }

            // Espacio doble -> inválido
            try {
                Etiqueta.crear("mal  formato");
                System.out.println("ERROR: etiqueta con espacio doble no lanzó excepción");
            } catch (IllegalArgumentException ex) {
                System.out.println("  [OK] Detectada etiqueta con espacio doble: " + ex.getMessage());
            }

            // Caracter especial -> inválido
            try {
                Etiqueta.crear("mal@formato");
                System.out.println("ERROR: etiqueta con carácter especial no lanzó excepción");
            } catch (IllegalArgumentException ex) {
                System.out.println("  [OK] Detectada etiqueta con carácter especial: " + ex.getMessage());
            }
        ...
        System.out.println("Tras agregar 'clave1' y 'clave2': " + a1.getPalabrasClave());
        ...
        try {
            ...
            System.out.println("  [OK] Detectada palabra clave demasiado larga");
        }

        // Espacio simple válido? No permitido en palabras clave
        try {
            a1.agregarPalabraClave("mal formato");
            System.out.println("ERROR: palabra clave con espacio no lanzó excepción");
        } catch (Exception ex) {
            System.out.println("  [OK] Detectada palabra clave con espacio: " + ex.getMessage());
        }
        ...
        Archivo a2 = new Archivo("archivo", a1.getRutaCompleta().toUpperCase(), "txt",
                2048, LocalDateTime.now(), LocalDateTime.now());
        System.out.println("\nComparación de rutas case-insensitive:");
        System.out.println("Rutas iguales ignoreCase: " +
